...
export default function OSCDashboard() {
  ...
  async function loadData() {
    ...
    const [donorsRes, donationsRes, alertsRes, casesRes, umaRes] =
      await Promise.all([
        supabase
          .from("donors")
          .select("*")
  ...
  const total6m = donations
    .filter((d) => new Date(d.donation_date) >= sixMonthsAgo)
    .reduce((s, d) => s + (d.amount_mxn || 0), 0);
  ...
  donors.forEach((d) => {
    if (statusCounts[d.compliance_status] !== undefined)
      statusCounts[d.compliance_status]++;
  });
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8">
      <PageHeader
        ...
        badge={
          <>
            <Shield className="w-3 h-3" /> Cumplimiento AML
          </>
        }
        actions={
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          subtitle={`${formatUMA(mxnToUMA(total6m, umaConfig))} acumuladas`}
          icon={TrendingUp}
          color="violet"
        />
        <StatCard
          title="Donantes Activos"
          value={donors.filter((d) => d.status !== "blocked").length}
          subtitle={`${donors.length} registrados total`}
          icon={Users}
          color="green"
        />
        <StatCard
          ...
          value={alerts.length}
          subtitle={`${criticalAlerts.length} críticas`}
          icon={Bell}
          color="red"
        />
      </div>
              ...
              <div key={key} className="flex items-center gap-3">
                ...
                <div className="w-20">
                  <Progress
                    value={
                      donors.length
                        ? (statusCounts[key] / donors.length) * 100
                        : 0
                    }
                    className="h-1.5"
                  />
              ...
              {pendingCases.slice(0, 4).map((c) => {
                ...
                return (
                  ...
                  >
                    ...
                    {days !== null && (
                      <span
                        className={`text-xs font-semibold px-2 py-1 rounded-lg ${days <= 3 ? "bg-red-100 text-red-700" : days <= 7 ? "bg-amber-100 text-amber-700" : "bg-slate-100 text-slate-600"}`}
                      >
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          ...
          <div className="space-y-2.5">
            {alerts.slice(0, 5).map((a) => (
              ...
              >
                <div
                  className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${a.severity === "critical" ? "bg-red-500" : a.severity === "high" ? "bg-orange-500" : "bg-amber-500"}`}
                />
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <h2 className="font-semibold text-slate-900">
            Donantes — Estatus de Cumplimiento
          </h2>
          <Link to={createPageUrl("OSCDonors")}>
                ...
                return (
                  ...
                  >
                    <td className="px-6 py-3.5">
                      <Link
                        to={createPageUrl(`OSCDonors`)}
                        className="font-medium text-slate-900 text-sm hover:text-violet-700"
                      >
                        {d.full_name}
                      </Link>
                    ...
                    <td className="px-6 py-3.5">
                      <StatusBadge
                        status={d.compliance_status || "incomplete"}
                      />
                    </td>
                  </tr>
                );
