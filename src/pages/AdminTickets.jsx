...
export default function AdminTickets() {
  ...
  async function loadData() {
    ...
    const [{ data: tl }, { data: ol }] = await Promise.all([
      supabase
  ...
  async function createTicket(e) {
    e.preventDefault();
    await supabase.from("support_tickets").insert([{ ...form, notes: [] }]);
    setForm({
      title: "",
      description: "",
      category: "compliance",
  ...
  const statusColors = {
    ...
    pending_osc: "bg-amber-100 text-amber-700",
          ...
          >
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setForm((p) => ({ ...p, organization_id: v }))
                }
              >
                <SelectTrigger>
                  <SelectValue placeholder="Seleccionar OSC" />
                </SelectTrigger>
                <SelectContent>
                  {orgs.map((o) => (
            ...
            <div>
              ...
              >
                ...
                <SelectContent>
                  {["compliance", "technical", "billing", "general"].map(
                    (c) => (
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between mb-1.5">
                        ...
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full font-medium ml-2 flex-shrink-0 ${statusColors[t.status]}`}
                        >
                      ...
                      <div className="flex items-center gap-3 text-xs text-slate-500">
                        ...
                        <span
                          className={`font-medium ${priorityColors[t.priority]}`}
                        >
                          {t.priority}
                        </span>
                        <span>
                          {new Date(t.created_date).toLocaleDateString("es-MX")}
                        </span>
                      </div>
                    </button>
                  );
            ...
            {!loading && filtered.length === 0 && (
              <p className="text-center py-8 text-slate-400">Sin tickets</p>
            )}
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
            <div>
              <div className="flex items-start justify-between mb-2">
                ...
                <span
                  className={`text-xs px-2 py-0.5 rounded-full font-medium ${statusColors[selectedTicket.status]}`}
                >
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {selectedTicket.status === "in_progress" && (
                <Button
                  size="sm"
                  onClick={() => updateStatus(selectedTicket.id, "pending_osc")}
                  variant="outline"
              ...
              {["in_progress", "pending_osc"].includes(
                selectedTicket.status,
              ) && (
                <Button
                  size="sm"
                  onClick={() => updateStatus(selectedTicket.id, "resolved")}
                  className="bg-emerald-600 text-white h-7 text-xs"
                >
                  Resolver
                </Button>
              )}
            ...
            <div>
                ...
                {(selectedTicket.notes || []).map((n, i) => (
                  ...
                  >
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      {n.author} · {new Date(n.date).toLocaleString("es-MX")}
                    </p>
                  </div>
                ))}
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  onKeyDown={(e) =>
                    e.key === "Enter" && addNote(selectedTicket.id)
                  }
                />
                <Button
                  size="sm"
                  onClick={() => addNote(selectedTicket.id)}
                  className="bg-slate-900 text-white h-9"
                >
                  Agregar
                </Button>
              </div>
