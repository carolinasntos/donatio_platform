...
export default function DonorPortal() {
  ...
  const completeness = donor
    ? checkDocumentCompleteness(
        documents,
        donor.donor_type,
        donor.beneficial_owner_exists,
      )
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-6">
        ...
        {!donor && !showRegisterForm && !showLoginForm && (
          <div className="bg-white/10 backdrop-blur rounded-2xl border border-white/15 p-8 text-center">
            ...
            <h2 className="text-2xl font-bold text-white mb-2">
              Bienvenido al Portal de Donantes
            </h2>
            <p className="text-teal-200 mb-6 max-w-md mx-auto">
              Registra tu información para comenzar el proceso de donación de
              manera transparente y conforme a la normativa AML.
            </p>
            <Button
              ...
              <div className="flex gap-3">
                {["fisica", "moral"].map((t) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      setRegisterForm((p) => ({ ...p, donor_type: t }))
                    }
        ...
        {donor && (
          <>
            {/* Status card */}
            <div
              className={`rounded-2xl p-5 border ${donor.status === "blocked" ? "bg-red-900/30 border-red-500/30" : donor.kyc_complete ? "bg-emerald-900/20 border-emerald-500/30" : "bg-white/10 border-white/15"}`}
            >
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-teal-300 text-sm mt-0.5">
                    RFC: {donor.rfc || "—"}
                  </p>
                </div>
                <span
            ...
            {!completeness.isComplete && (
              <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5">
                ...
                <p className="text-sm text-amber-700 mb-3">
                  Para procesar donativos, necesitamos los siguientes
                  documentos:
                </p>
                <div className="flex flex-wrap gap-2">
                  {completeness.missing.map((m) => (
                    <span
                      key={m}
                      className="text-xs px-2.5 py-1 bg-amber-100 text-amber-800 rounded-full border border-amber-200"
                    >
            ...
            {donor.status === "blocked" && (
              <Alert className="border-red-300 bg-red-50">
                ...
                <AlertDescription className="text-red-700 font-medium">
                  Tu cuenta ha sido bloqueada. Por favor contacta a la
              ...
              <div className="bg-white rounded-2xl p-6 shadow-xl">
                ...
                {donateForm.payment_method === "efectivo" && (
                  <Alert className="mb-4 border-red-300 bg-red-50">
                    <AlertCircle className="w-4 h-4 text-red-600" />
                    <AlertDescription className="text-red-700 font-medium">
                      ⛔ Los donativos en efectivo no están permitidos por la
                      normativa AML.
                    </AlertDescription>
                  </Alert>
                )}
                <form onSubmit={handleDonate} className="space-y-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      min="1"
                      step="0.01"
                      value={donateForm.amount_mxn}
                      onChange={(e) =>
                        setDonateForm((p) => ({
                          ...p,
                          amount_mxn: e.target.value,
                        }))
                      }
                      required
                      placeholder="0.00"
                  ...
                  <div>
                    ...
                    <Select
                      value={donateForm.payment_method}
                      onValueChange={(v) =>
                        setDonateForm((p) => ({ ...p, payment_method: v }))
                      }
                    >
                      <SelectTrigger>
                        <SelectValue />
                  ...
                  <div className="flex gap-3 justify-end">
                    ...
                    <Button
                      type="submit"
                      disabled={
                        saving || donateForm.payment_method === "efectivo"
                      }
                      className="bg-teal-600 hover:bg-teal-700 text-white"
                    >
            ...
            <div className="bg-white/10 backdrop-blur rounded-2xl border border-white/15 p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <Link to={createPageUrl("DonorDonations")}>
                  <Button
                    size="sm"
                    variant="ghost"
                    className="text-teal-300 hover:text-white text-xs h-7"
                  >
                    Ver todo
                  </Button>
                </Link>
              ...
              <div className="space-y-2">
                {donations.slice(0, 5).map((d) => (
                  <div
                    key={d.id}
                    className="flex items-center justify-between p-3 rounded-xl bg-white/8"
                  >
                    <div>
                      ...
                      <p className="text-teal-300 text-xs">
                        {d.donation_date} · {d.payment_method}
                      </p>
                    </div>
                ...
                {donations.length === 0 && (
                  <p className="text-center py-6 text-teal-400 text-sm">
