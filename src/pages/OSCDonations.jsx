...
import {
  mxnToUMA, calculate6MonthRolling, formatCurrency, formatUMA,
  UMA_DEFAULT, checkThresholds, getDeadlineDate
...
export default function OSCDonations() {
  ...
  const [form, setForm] = useState({ donor_id: "", amount_mxn: "", donation_date: new Date().toISOString().split("T")[0], payment_method: "transferencia", description: "", reference: "", is_manual_entry: true });
  ...
  function handleAmountChange(val) {
    ...
    if (form.donor_id && val) {
      ...
      setThresholdWarning(trigger !== 'none' ? { trigger, newTotal, newUMA } : null);
    }
  }
  ...
  async function handleSubmit(e) {
    ...
    setSaving(false);
    setShowForm(false);
    setForm({ donor_id: "", amount_mxn: "", donation_date: new Date().toISOString().split("T")[0], payment_method: "transferencia", description: "", reference: "", is_manual_entry: true });
  ...
  const filtered = donations.filter(d => {
    ...
    return donor?.full_name?.toLowerCase().includes(search.toLowerCase()) || d.reference?.toLowerCase().includes(search.toLowerCase());
  });
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <PageHeader
        ...
        actions={
          <Button onClick={() => setShowForm(!showForm)} className="bg-slate-900 hover:bg-slate-800 text-white gap-2">
      ...
      {showForm && (
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          <h3 className="font-semibold text-slate-900 mb-4">Nuevo Donativo (Captura Manual)</h3>
          {cashWarning && (
            <Alert className="mb-4 border-red-300 bg-red-50">
              <AlertCircle className="w-4 h-4 text-red-600" />
              <AlertDescription className="text-red-700 font-medium">
                ⛔ BLOQUEADO: Los donativos en efectivo no están permitidos bajo la normativa AML/LFPIORPI. Selecciona otro método de pago.
              </AlertDescription>
            </Alert>
          )}
          {thresholdWarning && (
            <Alert className={`mb-4 ${thresholdWarning.trigger === 'notice' ? 'border-red-300 bg-red-50' : 'border-amber-300 bg-amber-50'}`}>
              <AlertCircle className={`w-4 h-4 ${thresholdWarning.trigger === 'notice' ? 'text-red-600' : 'text-amber-600'}`} />
              <AlertDescription className={thresholdWarning.trigger === 'notice' ? 'text-red-700' : 'text-amber-700'}>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Label className="text-xs font-semibold text-slate-700 mb-1.5 block">Donante *</Label>
              <Select value={form.donor_id} onValueChange={v => setForm(p => ({ ...p, donor_id: v }))}>
                <SelectTrigger><SelectValue placeholder="Seleccionar donante" /></SelectTrigger>
                <SelectContent>
                  {donors.filter(d => d.status !== 'blocked').map(d => (
                    <SelectItem key={d.id} value={d.id}>{d.full_name} — {d.rfc || "Sin RFC"}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            <div>
              ...
              <Input type="number" min="0" step="0.01" value={form.amount_mxn} onChange={e => handleAmountChange(e.target.value)} required placeholder="0.00" />
            </div>
            <div>
              <Label className="text-xs font-semibold text-slate-700 mb-1.5 block">Fecha del Donativo *</Label>
            ...
            <div>
              <Label className="text-xs font-semibold text-slate-700 mb-1.5 block">Referencia / Folio</Label>
            ...
            <div className="md:col-span-2 flex justify-end gap-3 pt-2 border-t border-slate-100">
              <Button type="button" variant="outline" onClick={() => { setShowForm(false); setCashWarning(false); setThresholdWarning(null); }}>Cancelar</Button>
              <Button type="submit" disabled={cashWarning || saving} className="bg-slate-900 hover:bg-slate-800 text-white">
      ...
      <div className="relative max-w-sm">
        ...
        <Input placeholder="Buscar donativo..." value={search} onChange={e => setSearch(e.target.value)} className="pl-9 h-9" />
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="text-left px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Monto</th>
                <th className="text-left px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Método</th>
                <th className="text-left px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Acum. 6M</th>
                <th className="text-left px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Umbral</th>
            ...
            <tbody>
              {loading ? (
                ...
                return (
                  <tr key={don.id} className="border-b border-slate-50 hover:bg-slate-50/60">
                    <td className="px-6 py-3.5 text-sm font-medium text-slate-900">{donor?.full_name || don.donor_id}</td>
                    <td className="px-6 py-3.5 text-sm text-slate-600">{don.donation_date}</td>
                    <td className="px-6 py-3.5 text-sm font-bold text-slate-900">{formatCurrency(don.amount_mxn)}</td>
                    <td className="px-6 py-3.5">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${don.payment_method === 'efectivo' ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-600'}`}>
                        {don.payment_method}
                      </span>
                    </td>
                    <td className="px-6 py-3.5 text-sm text-slate-700">{formatCurrency(don.accumulated_6m_at_donation)}</td>
                    <td className="px-6 py-3.5">
                      {don.threshold_triggered === 'notice' && <Badge className="bg-red-100 text-red-700 border border-red-200">Aviso SAT</Badge>}
                      {don.threshold_triggered === 'identification' && <Badge className="bg-amber-100 text-amber-700 border border-amber-200">Identificación</Badge>}
                      {(!don.threshold_triggered || don.threshold_triggered === 'none') && <span className="text-xs text-slate-400">—</span>}
                    </td>
                    <td className="px-6 py-3.5 text-sm text-slate-500 font-mono">{don.reference || "—"}</td>
                  </tr>
                );
              })}
              {!loading && filtered.length === 0 && (
                <tr><td colSpan={7} className="px-6 py-12 text-center text-slate-400">No hay donativos registrados</td></tr>
              )}
            </tbody>
          </table>
        </div>
