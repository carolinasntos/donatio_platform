...
export default function DonorDocuments() {
    ...
    return (
      <div className="p-6 max-w-2xl mx-auto text-center py-20">
        <p className="text-slate-500">
          Primero debes registrarte como donante desde el Portal Donante.
        </p>
      </div>
    );

  const required = donor
    ? Object.entries(DOCUMENT_TYPES)
        .filter(([, c]) => c.required_for.includes(donor.donor_type))
        .map(([k]) => k)
    : [];
  if (donor?.beneficial_owner_exists)
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Expediente digital requerido para cumplimiento AML
          </p>
        </div>
        <Button
      ...
      {/* Completeness */}
      <div
        className={`p-4 rounded-2xl border ${completeness.isComplete ? "bg-emerald-50 border-emerald-200" : "bg-amber-50 border-amber-200"}`}
      >
          ...
          {completeness.isComplete ? (
            ...
            <>
              ...
              <span className="font-semibold text-amber-800">
                {completeness.missing.length} documento(s) pendiente(s)
              </span>
            </>
          )}
        </div>
      ...
      {showUpload && (
        ...
        >
              ...
              >
                ...
                <SelectContent>
                  {Object.entries(DOCUMENT_TYPES)
                    .filter(
                      ([, c]) =>
                        !c.required_for.length ||
                        c.required_for.includes(donor?.donor_type),
                    )
                    .map(([k, v]) => (
          ...
          <div className="flex gap-2 justify-end">
            ...
            <Button
              type="submit"
              disabled={uploading || !form.document_type}
        ...
        {loading
          ...
          : required.map((reqType) => {
              ...
              return (
                <div
                  key={reqType}
                  className={`flex items-center justify-between p-4 rounded-xl border ${latestDoc && !isExpired ? "bg-emerald-50 border-emerald-200" : isExpired ? "bg-red-50 border-red-200" : "bg-slate-50 border-slate-200"}`}
                >
