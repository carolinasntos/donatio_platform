...
import { Shield, ArrowRight, CheckCircle2, Building2, Users, AlertTriangle, FileText } from "lucide-react";
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-violet-950 text-white">
      ...
      <nav className="flex items-center justify-between px-8 py-5 max-w-7xl mx-auto">
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold text-xl tracking-tight">AppleSeed AML</span>
        </div>
        <div className="flex items-center gap-3">
          <Link to={createPageUrl("OSCDashboard")}>
            <Button variant="ghost" className="text-white/70 hover:text-white hover:bg-white/10">Portal OSC</Button>
          </Link>
          <Link to={createPageUrl("AdminDashboard")}>
            <Button variant="ghost" className="text-white/70 hover:text-white hover:bg-white/10">AppleSeed Admin</Button>
          </Link>
          <Link to={createPageUrl("DonorPortal")}>
            <Button className="bg-violet-600 hover:bg-violet-500 text-white border-0">Portal Donante</Button>
          </Link>
        </div>
      </nav>
      ...
      <main className="max-w-7xl mx-auto px-8 pt-20 pb-32">
        <div className="max-w-3xl">
          ...
          <p className="text-xl text-white/60 leading-relaxed mb-10 max-w-2xl">
            Plataforma integral para OSC. Controla acumulados en ventana móvil de 6 meses, genera alertas automáticas, organiza expedientes digitales y prepara la Plantilla SAT para avisos en tiempo.
          </p>
          <div className="flex flex-wrap gap-4">
            <Link to={createPageUrl("OSCDashboard")}>
              <Button size="lg" className="bg-violet-600 hover:bg-violet-500 text-white gap-2 px-8 h-12">
                Acceder al Portal OSC <ArrowRight className="w-4 h-4" />
              </Button>
            </Link>
            <Link to={createPageUrl("DonorPortal")}>
              <Button size="lg" variant="outline" className="border-white/30 text-white bg-white/10 hover:bg-white/20 gap-2 px-8 h-12">
                Soy Donante
              </Button>
            </Link>
          </div>
        ...
        <div className="mt-20 grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { title: "Portal OSC", subtitle: "Para organizaciones sin fines de lucro", page: "OSCDashboard", color: "from-slate-800 to-slate-700", border: "border-slate-600" },
            { title: "Portal Donante", subtitle: "Para personas físicas y morales", page: "DonorPortal", color: "from-teal-900 to-teal-800", border: "border-teal-700" },
            { title: "AppleSeed Admin", subtitle: "Supervisión y control multi-OSC", page: "AdminDashboard", color: "from-violet-900 to-violet-800", border: "border-violet-700" },
          ].map(({ title, subtitle, page, color, border }) => (
            <Link key={page} to={createPageUrl(page)} className={`group p-6 rounded-2xl bg-gradient-to-br ${color} border ${border} hover:scale-[1.02] transition-transform`}>
