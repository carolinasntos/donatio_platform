  ...
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      content:
        "Hola, soy tu asistente de cumplimiento AML. Puedo ayudarte con preguntas sobre la LFPIORPI y tus obligaciones como OSC que recibe donativos. ¿En qué te puedo orientar?\n\n*Recuerda: Esta orientación es informativa y no sustituye asesoría legal profesional.*",
    },
  ]);
    ...
    >
              ...
              setMessages([
                {
                  role: "assistant",
                  content:
                    "Nueva conversación iniciada. ¿En qué te puedo orientar sobre cumplimiento AML?",
                },
              ])
            }
            className="gap-2 h-8"
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-2.5 flex items-center gap-2 mb-4">
        ...
        <p className="text-xs text-amber-700">
          Este chatbot es orientativo. No sustituye asesoría legal profesional.
          Para casos complejos, consulta con tu asesor jurídico especializado en
          AML.
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {FAQ_PROMPTS.map((faq) => (
          <button
            ...
            className="text-xs px-3 py-1.5 rounded-full bg-slate-100 text-slate-600 hover:bg-slate-200 transition-colors border border-slate-200"
          >
            {faq}
          </button>
        ))}
      ...
      <div className="flex-1 overflow-y-auto bg-white rounded-2xl border border-slate-200 p-4 space-y-4 mb-4">
        {messages.map((msg, i) => (
          ...
          >
            ...
            <div
              className={`max-w-[80%] rounded-2xl px-4 py-3 ${msg.role === "user" ? "bg-slate-900 text-white" : "bg-slate-50 border border-slate-200 text-slate-800"}`}
            >
              {msg.role === "user" ? (
      ...
      <div className="flex gap-3">
        ...
        <Button
          ...
          className="bg-slate-900 hover:bg-slate-800 text-white self-end h-10 w-10 p-0"
        >
