...
export default function OSCDonors() {
  ...
  const filtered = donors.filter((d) => {
    ...
    const matchStatus =
      filterStatus === "all" || d.compliance_status === filterStatus;
  ...
  async function handleSave(data) {
    if (editingDonor) {
      await supabase.from("donors").update(data).eq("id", editingDonor.id);
    } else {
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <Select value={filterStatus} onValueChange={setFilterStatus}>
          ...
          <SelectContent>
            ...
            <SelectItem value="identification_required">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <div className="px-6 py-3 bg-slate-50 border-b border-slate-100 text-xs text-slate-500 font-medium">
          {filtered.length} donante{filtered.length !== 1 ? "s" : ""} encontrado
          {filtered.length !== 1 ? "s" : ""}
        </div>
                      ...
                      >
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div
                              className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${d.donor_type === "moral" ? "bg-blue-100" : "bg-violet-100"}`}
                            >
                              {d.donor_type === "moral" ? (
