...
export default function OSCSettings() {
  ...
  async function handleSave(e) {
    ...
    if (umaConfig?.id) {
      await supabase.from("uma_config").update(data).eq("id", umaConfig.id);
    } else {
      await supabase.from("uma_config").insert(data);
    }
    setSaving(false);
    setSaved(true);
  ...
  const threshold1MXN =
    annual * parseFloat(form.threshold_identification_uma || 0);
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      <PageHeader
        title="Configuración"
        subtitle="Parámetros normativos y valores UMA"
      />
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6">
        ...
        <form onSubmit={handleSave} className="space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <Label className="text-xs font-semibold text-slate-700 mb-1.5 block">
                Umbral 2: Obligación de Aviso (UMAs)
              </Label>
              <Input
                type="number"
                ...
                onChange={(e) =>
                  setForm((p) => ({
                    ...p,
                    threshold_notice_uma: e.target.value,
                  }))
                }
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div className="bg-amber-50 rounded-lg p-3 border border-amber-200">
                ...
                <p className="font-bold text-amber-900">
                  {formatCurrency(threshold1MXN)}
                </p>
                <p className="text-xs text-amber-600">
                  {form.threshold_identification_uma} UMAs
                </p>
              </div>
              <div className="bg-red-50 rounded-lg p-3 border border-red-200">
                <p className="text-xs text-red-600 mb-1">Umbral 2 en MXN</p>
                <p className="font-bold text-red-900">
                  {formatCurrency(threshold2MXN)}
                </p>
                <p className="text-xs text-red-600">
                  {form.threshold_notice_uma} UMAs
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6">
        <h2 className="font-bold text-slate-900 mb-4">
          Reglas Normativas LFPIORPI
        </h2>
        <div className="space-y-3 text-sm text-slate-600">
          {[
            {
              icon: "📅",
              text: "Ventana de acumulación: 6 meses móviles por donante.",
            },
            {
              icon: "⚠️",
              text: `Al superar ${form.threshold_identification_uma} UMAs (${formatCurrency(threshold1MXN)}): se activa Identificación Reforzada del donante.`,
            },
            {
              icon: "🚨",
              text: `Al superar ${form.threshold_notice_uma} UMAs (${formatCurrency(threshold2MXN)}): se activa Obligación de Aviso al SAT.`,
            },
            {
              icon: "📆",
              text: "Plazo para presentar aviso: día 17 del mes siguiente a cuando se rebasó el umbral.",
            },
            {
              icon: "⛔",
              text: "Donativos en efectivo: PROHIBIDOS. Se genera alerta crítica y bloqueo inmediato.",
            },
            {
              icon: "📋",
              text: "Expediente del donante debe mantenerse completo antes de aceptar donativos.",
            },
          ].map(({ icon, text }, i) => (
            <div
              key={i}
