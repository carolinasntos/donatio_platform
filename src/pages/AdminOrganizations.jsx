...
export default function AdminOrganizations() {
  ...
  async function handleSave(e) {
    ...
    setSaving(true);
    if (editing) {
      await supabase.from("organizations").update(form).eq("id", editing.id);
    } else {
  ...
  const filtered = orgs.filter(
    (o) =>
      ...
      o.rfc?.toLowerCase().includes(search.toLowerCase()),
  );
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          <div className="flex items-center justify-between mb-5">
            <h3 className="font-bold text-slate-900">
              {editing ? "Editar OSC" : "Registrar Nueva OSC"}
            </h3>
            <Button
              variant="ghost"
              size="icon"
          ...
          >
            ...
            <div>
              ...
              <Select
                value={form.moral_type}
                onValueChange={(v) => setForm((p) => ({ ...p, moral_type: v }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm((p) => ({ ...p, legal_rep_name: e.target.value }))
                }
              />
            </div>
            <div>
            ...
            <div>
              ...
              >
                ...
                <SelectContent>
                  {["trial", "basic", "professional", "enterprise"].map((t) => (
              ...
              {loading
                ...
                : filtered.map((org) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-3.5">
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full font-medium ${org.status === "active" ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}
                        >
                          {org.status === "active" ? "Activa" : org.status}
                        </span>
                      </td>
                      <td className="px-6 py-3.5 text-xs text-slate-500">
                        {org.created_date?.split("T")[0]}
                      </td>
                      <td className="px-6 py-3.5">
                        <Button
                          size="sm"
